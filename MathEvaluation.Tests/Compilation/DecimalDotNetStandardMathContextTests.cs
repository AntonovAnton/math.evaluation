...
public class DecimalDotNetStandardMathContextTests(ITestOutputHelper testOutputHelper)
{
       ...
       double expectedValue, double var_a, double var_b = 0d, double var_c = 0d, double var_d = 0d)
    {
        ...
        var fn = expression.CompileDecimal(dict, _context, CultureInfo.InvariantCulture);
        var value = fn(dict);
    ...
    public void MathExpression_CompileDecimalThenInvoke_ReturnsNanOrInfinity_ThrowOverflowException(string expression)
    {
        ...
        var ex = Record.Exception(() => new MathExpression(expression, _context, CultureInfo.InvariantCulture).CompileDecimal()());
