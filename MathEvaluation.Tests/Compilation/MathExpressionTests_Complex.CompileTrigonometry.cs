...
public partial class MathExpressionTests_Complex
{
    ...
    public void MathExpression_CompileComplexThenInvoke_HasPi_ExpectedValue(string mathString, double expectedValue)
    {
    ...
    [InlineData("+ττ", Math.Tau * Math.Tau)]
    public void MathExpression_CompileComplexThenInvoke_HasTau_ExpectedValue(string mathString, double expectedValue)
    {
        using var expression = new MathExpression(mathString, _scientificContext, CultureInfo.InvariantCulture);
        expression.Evaluating += SubscribeToEvaluating;
