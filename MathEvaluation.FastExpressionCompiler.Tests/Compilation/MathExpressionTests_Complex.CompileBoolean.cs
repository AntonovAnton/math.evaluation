...
public partial class MathExpressionTests_Complex
{
    ...
    public void FastMathExpression_CompileComplexThenInvoke_HasProgrammingBooleanLogic_ExpectedValue(string mathString, bool expectedValue)
    {
    ...
    [InlineData("4 ≠ 4 OR 5.4 = 5.4 AND NOT true", false)]
    [InlineData("4 ≠ 4 OR 5.4 = 5.4 AND NOT 0 < 1 XOR 1.0 - 1.95 * 2 ⪰ -12.9 + 0.1 / 0.01", true)]
    public void FastMathExpression_CompileComplexThenInvoke_HasEngineeringBooleanLogic_ExpectedValue(string mathString, bool expectedValue)
    {
    ...
    public void FastMathExpression_CompileComplexThenInvoke_HasScientificBooleanLogic_ExpectedValue(string mathString, bool expectedValue)
    {
        using var expression = new FastMathExpression(mathString, _scientificContext, CultureInfo.InvariantCulture);
        expression.Evaluating += SubscribeToEvaluating;
    ...
    public void FastMathExpression_CompileComplexThenInvoke_HasVariables_ExpectedValue(string expression, bool a, bool b, bool c, bool expectedValue)
    {
        testOutputHelper.WriteLine($"{expression} = {expectedValue}");

        var getC = () => c;
        var fn = expression.CompileComplexFast(new { A = a, B = b, C = getC }, _programmingContext);
    ...
    public void FastMathExpression_CompileComplexThenInvoke_HasCustomFunction_ExpectedValue(string expression, double a, bool expectedValue)
    {
        testOutputHelper.WriteLine($"{expression} = {expectedValue}");
        ...
        var fn = expression.CompileComplexFast(new { a }, context);
        var value = fn(new { a });
    ...
    public void FastMathExpression_CompileComplexThenInvoke_HasIif_ExpectedValue(string expression, double a, bool expectedValue)
    {
        testOutputHelper.WriteLine($"{expression} = {expectedValue}");

        var fn = expression.CompileComplexFast(new { a }, _programmingContext);
        var value = fn(new { a });
